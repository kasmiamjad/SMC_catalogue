...
function loginAndGetSessionCookie() {
    $loginUrl = 'https://smc.ardhalfan.com/web/session/mobile_login?db=Production';
...
function fetchData($sessionCookie) {
    ...
    curl_setopt_array($curl, array(
        CURLOPT_URL => 'https://smc.ardhalfan.com/web/get_external_product_stock_details',
...
function replaceImageUrls($data) {
    $ipPattern = '/http:\/\/144\.91\.84\.159:8069/';
    $correctUrl = 'https://smc.ardhalfan.com';
    ...
    // Recursively replace URLs in the array
    array_walk_recursive($decodedData, function (&$value) use ($ipPattern, $correctUrl) {
        if (is_string($value) && preg_match($ipPattern, $value)) {
            $value = preg_replace($ipPattern, $correctUrl, $value);
        }
    });
...
$updatedData = replaceImageUrls($data);

if (!file_put_contents('data.json', $updatedData)) {
