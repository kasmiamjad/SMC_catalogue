						...
						<div class="container">
							<div class="row">
								...
								<div class="col col-12">
                                        ...
                                        <!-------------------------------------------------------- Products ---------------------------------------------------------->
										<div id="productsContainer" style="height:auto;" class="lqd-pf-row row flex flex-wrap -mr-15 -ml-15" >
...
<!-------------------------------------------------------- CODE ---------------------------------------------------------->
        ...
        <script>
           ...
           document.addEventListener("DOMContentLoaded", function() {
                ...
                let selectedCategory = ''; /// NEWWWWWWWWWWWWWW
                ...
                document.querySelectorAll('.category-link').forEach(function(item) {
                    item.addEventListener('click', function(e) {
                        ...
                        createSearchBar();
                        loadProducts(selectedCategory, 1); // NEWWWWWWWWWWWWWW
                    });
                });
        
                        function loadProducts(category, page) {
                             fetch(`load_products_2.php?category=${encodeURIComponent(category)}&page=${page}&limit=200`)
                            ...
                            .then(responseData => {
                                ...
                                // After adding all products, append the footer div
                                const footerHTML = `
                                 
                                `;
                                 updatePagination(responseData.totalPages, responseData.currentPage);
                                productsContainer.innerHTML += footerHTML;
                        ...
                        function updatePagination(totalPages, currentPage) {
                            ...
                            if (currentPage > 1) {
                                ...
                                prevButton.onclick = () => loadProducts(selectedCategory, currentPage - 1);
                                paginationContainer.appendChild(prevButton);
                            }
                        
                            // Page Number Buttons (optional, for showing individual page numbers)
                            for (let i = 1; i <= totalPages; i++) {
                                const pageButton = document.createElement('button');
                                pageButton.textContent = i;
                                pageButton.disabled = i === currentPage;
                                pageButton.onclick = () => loadProducts(selectedCategory, i);
                            ...
                            if (currentPage < totalPages) {
                                ...
                                nextButton.onclick = () => loadProducts(selectedCategory, currentPage + 1);
                        ...
                        function createPaginationContainer() {
                            ...
                            container.style.textAlign = 'center'; // Style as needed
                            // Append the container to a specific part of your page, like the bottom of the product list
        ...
        <script>
		    function createSearchBar() {
                ...
                if (!document.getElementById('searchContainer')) {
                    ...
                    input.onkeyup = searchProducts; // Assuming searchProducts is your search function
        ...
        <script>
            ...
            for (let i = 0; i < products.length; i++) {
                // Access the elements for title, reference, description, and quantity using the new class names
                let title = products[i].querySelector(".lqd-pf-title");
                let ref = products[i].querySelector(".ref_product"); // Using the new class for reference
                let description = products[i].querySelector(".description_product"); // Using the new class for description
                let quantity = products[i].querySelector(".quantity_product"); // Using the new class for quantity
        
                // Combine text values from all these elements
                let textValues = [title, ref, description, quantity].filter(element => element).map(element => element.textContent || element.innerText).join(" ");
